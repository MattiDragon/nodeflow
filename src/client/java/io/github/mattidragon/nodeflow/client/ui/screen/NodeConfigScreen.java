...
public class NodeConfigScreen<T extends Node> extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.fill(x - 2, 18, width - 18, height - 18, 0x33ffffff);

        if (!texts.isEmpty()) {
            graphics.text(font, Component.translatable("node.nodeflow.generic.config.errors"), x, 20, 0xffff5555, false);
        } else {
            graphics.text(font, Component.translatable("node.nodeflow.generic.config.no_errors"), x, 20, 0xff55ff55, false);
        }
        ...
        graphics.text(font, text, (width - 200 - font.width(text.getVisualOrderText())) / 2, 10, 0xffffffff, false);
    }
